...
--
--  Andrew Hall, Australia, June 2017
--
...
package body Vectors_xD is
   ...
   function Image (V : Vector_xD) return String is
      ...
      Image_String : Strings_255.Bounded_String :=
   ...
   begin
      for Axes in Vector_xD'Range loop
         ...
         if Axes /= Vector_xD'Last then
            Image_String := Image_String & Character'Val (9);
         end if;
      end loop;
   ...
   function "*" (V : Vector_xD; Scalar : Real) return Vector_xD is
   ...
   begin
      if Abs_Left = 0.0 or else Abs_Right = 0.0 then
         ...
         return Arccos (Real'Max (-1.0, Real'Min (1.0, (V_Left * V_Right)
                / (Abs_Left * Abs_Right))));
      end if;
   end Angle_Between;
